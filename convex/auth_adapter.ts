// Phase 3 — Option 2: Public auth adapter wrappers for Next.js.
//
// These functions call the Better Auth component's internal adapter.
// The component adapter has narrow types; we use `any` casts here
// because the generic args (v.string(), v.any()) are validated at
// runtime by the component's own validators.
...
export const findOne = query({
  ...
  handler: async (ctx, args) => {
    return (ctx.runQuery as any)(components.betterAuth.adapter.findOne, args);
  },
});
...
export const findMany = query({
  ...
  handler: async (ctx, args) => {
    return (ctx.runQuery as any)(components.betterAuth.adapter.findMany, args);
  },
});
